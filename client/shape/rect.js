...
Rect.prototype.contains = function (that) {
  ...
  return that.x >= this.getBBox().x && that.x <= this.getBBox().x2 &&
         that.y >= this.getBBox().y && that.y <= this.getBBox().y2;
}

Rect.prototype.mover = function (isEdge, cursor) {
  // Take a copy of the bbox to operate on
  var bbox = _.pick(this.getBBox(), 'x', 'y', 'width', 'height');
  if (!isEdge) { // Body
    return function (dx, dy) { return this.deltaBBox(bbox, { x : dx, y : dy }); };
  } else if (cursor.contains(new Point(this.getBBox().x, this.getBBox().y))) { // Top left corner
    return function (dx, dy) { return this.deltaBBox(bbox, { x : dx, y : dy, width : -dx, height : -dy }); };
  } else if (cursor.contains(new Point(this.getBBox().x2, this.getBBox().y))) { // Top right corner
    return function (dx, dy) { return this.deltaBBox(bbox, { y : dy, width : dx, height : -dy }); };
  } else if (cursor.contains(new Point(this.getBBox().x, this.getBBox().y2))) { // Bottom left corner
    return function (dx, dy) { return this.deltaBBox(bbox, { x : dx, width : -dx, height : dy }); };
  } else if (cursor.contains(new Point(this.getBBox().x2, this.getBBox().y2))) { // Bottom right corner
    return function (dx, dy) { return this.deltaBBox(bbox, { width : dx, height : dy }); };
  } else if (cursor.contains(new Point(this.getBBox().x, cursor.getBBox().c.y))) { // Left side
    return function (dx, dy) { return this.deltaBBox(bbox, { x : dx, width : -dx }); };
  } else if (cursor.contains(new Point(cursor.getBBox().c.x, this.getBBox().y))) { // Top side
    return function (dx, dy) { return this.deltaBBox(bbox, { y : dy, height : -dy }); };
  } else if (cursor.contains(new Point(this.getBBox().x2, cursor.getBBox().c.y))) { // Right side
    return function (dx, dy) { return this.deltaBBox(bbox, { width : dx }); };
  } else if (cursor.contains(new Point(cursor.getBBox().c.x, this.getBBox().y2))) { // Bottom side
