...
Linkarc.prototype.link = function (from, to) {
  ...
  var c1 = from.getBBox().c, c2 = to.getBBox().c, mc = c1.lerp(c2, 0.5),
...
Linkarc.prototype.withEnds = function (p1, p2) {
  if (p1.equals(this.getEnds()[0]) && p2.equals(this.getEnds()[1])) {
    return this;
  } else {
    ...
    return this.delta({ d : _.mapValues({
      ...
      '1.curve.largeArcFlag' : Arc.isLargeArc(this.getCentre(), p1, p2, curve.sweepFlag)
    }, _.constant) });
  }
};
...
Linkarc.prototype.mover = function (isEdge, cursor, getShapeById) {
  ...
  var traverse = _.bind(function (point) {
    return this.delta({ d : {
      '1.curve' : _.constant(Arc.curveTraversing(from.getBBox().c, point, to.getBBox().c))
    } }).link(from, to);
  }, this);
  if (isEdge) {
    ...
    return function (dx, dy) {
      ...
      var mc = this.getEnds()[0].add(this.chord.divide(2)),
          pv = this.chord.perp().unit().multiply(this.getExtent()),
